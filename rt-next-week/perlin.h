...
class perlin {
        ...
        for(int di = 0; di < 2; di++)
            for(int dj = 0; dj < 2; dj++)
                for(int dk = 0; dk < 2; dk++) {
                    auto idx = perm_x[(i + di) & 255] ^ perm_y[(j + dj) & 255] ^ perm_z[(k + dk) & 255];
                    c[di][dj][dk] = randvec[idx];
                }
  ...
  private:
    using cube = std::array<std::array<std::array<vec3, 2>, 2>, 2>;
    ...
    static void perlin_generate_perm(std::array<int, point_count>& p) {
        for(int i = 0; i < point_count; i++)
            p[i] = i;
    ...
    static double perlin_interp(const cube& c, double u, double v, double w) {
        auto uu = u * u * (3 - 2 * u);
        auto vv = v * v * (3 - 2 * v);
        auto ww = w * w * (3 - 2 * w);
        auto accum = 0.0;
